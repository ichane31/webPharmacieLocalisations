...
export default function AddPayment() {
  ...
  const onInputChange = async (e) => {
    ...
    const result = await axios.get(`http://localhost:8080/factureswater/amount/${e.target.value}`);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
            await axios.put(`http://localhost:8080/factureswater/pay/${reference}`,elecwater);
            navigate("/homenajia");    
  };
  return (
      ...
      <div className="row">
        <div className="col-md-6 offset-md-3 border rounded p-4 mt-2 shadow">
          ...
          <form onSubmit={(e) => onSubmit(e)}>
            ...
            <Link className="btn btn-outline-danger mx-2" to="/listefactureswater">
              Cancel
            </Link>
          </form>
        </div>
